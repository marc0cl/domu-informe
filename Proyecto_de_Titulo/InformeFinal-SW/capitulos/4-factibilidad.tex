...
A través de esta evaluación, se busca proporcionar una base sólida para la toma de decisiones estratégicas, garantizando que \textbf{DOMU} no solo sea viable desde el punto de vista conceptual, sino también aplicable y rentable en contextos reales. En este capítulo, se abordarán tres dimensiones fundamentales de la viabilidad del proyecto: la \textbf{factibilidad técnica}, la \textbf{factibilidad operativa} y, especialmente, la \textbf{factibilidad económica}, cuyo análisis permitirá establecer si los beneficios esperados justifican los costos asociados al desarrollo e implementación del sistema.
...
\begin{longtable}{|p{0.38\textwidth}|p{0.18\textwidth}|p{0.36\textwidth}|}
    ...
    \hline
    \multicolumn{3}{|c|}{\textbf{Especificación del software requerido en desarrollo (cont.)}} \\
    \hline
    \textbf{Software} & \textbf{Versión Mínima} & \textbf{Propósito en el Proyecto} \\
    \hline
    \endhead
    ...
    Javalin & 6 & Framework de aplicaciones Java \\ \hline

    \multicolumn{3}{|l|}{\textit{Tecnologías Front-End}} \\ \hline
    Node.js + NVM & LTS & Entorno de ejecución y gestión de versiones \\
    React (Web + PWA) & Actual & Framework para interfaces de usuario (web y móvil) \\ \hline
...
\begin{table}[H]
    ...
    \hline
    Pistola escáner de QR & Capaz de leer códigos QR (ISO/IEC 18004~\cite{iso18004}), conexión USB o Bluetooth. \\
    \hline
    \end{tabular}
\end{table}
...
\subsection*{Conclusión de la factibilidad técnica}
Con base en las especificaciones de software, hardware de desarrollo, la estrategia de servidores en producción y los planes de mantenimiento, se concluye que el proyecto \textbf{DOMU} es técnicamente factible.

El equipo de desarrollo cuenta con los recursos tecnológicos necesarios y un ecosistema de software moderno (Java, Javalin, React, MySQL y Docker). En cuanto al hardware, los componentes físicos requeridos para la operación (pistola escáner) son de fácil adquisición y alta compatibilidad.
...
El éxito operativo de una plataforma como \textbf{DOMU} está estrechamente vinculado al nivel de acceso a la tecnología y a las competencias digitales de sus usuarios. En este contexto, las condiciones actuales de conectividad en las regiones del Biobío y La Araucanía resultan favorables. De acuerdo con datos de Subtel~\cite{subtel2025} e INE, más del 80~\% de los hogares urbanos en estas regiones cuenta con conexión a Internet fija o móvil. Esto permite establecer que tanto residentes como miembros del comité poseen el equipamiento necesario para interactuar con la plataforma.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.20\textwidth}|c|r|c|r|c|r|}
    \hline
...
\subsection{Cálculo de la Tasa Interna de Retorno (TIR)}

La TIR es la tasa de descuento que iguala el VAN a cero. Se obtiene resolviendo:

$$
\sum_{t=0}^{n} \frac{F_t}{(1 + TIR)^t} = 0
$$

Mediante el método de interpolación con los flujos netos del proyecto, se obtiene una \textbf{TIR $\approx$ 125\%}. Dado que la TIR es significativamente superior a la tasa de descuento utilizada ($k = 10\%$), se confirma la viabilidad económica del proyecto.

\subsection{Supuestos y Limitaciones del Análisis}

El presente análisis de factibilidad económica se basa en los siguientes supuestos simplificadores:

\begin{itemize}
    \item \textbf{Ingresos:} La proyección de ingresos para los años~2 a~5 utiliza el número de suscriptores al final de cada año. Un cálculo más conservador utilizando el promedio de suscriptores del año (inicio~$+$~fin~/~2) reduciría los ingresos proyectados en aproximadamente un 15--20\%.
    \item \textbf{Costos operativos:} Se mantienen constantes en \$18.519.990/año durante los 5~años. En la práctica, los costos de soporte, hosting y mantenimiento crecerían proporcionalmente al número de comunidades atendidas.
    \item \textbf{Tasa de cancelación (\textit{churn}):} Se asume 0\% de cancelaciones. En modelos SaaS, una tasa de churn mensual del 1--3\% es habitual.
    \item \textbf{Impuestos:} No se consideran IVA (19\%), impuesto de primera categoría (27\%) ni patente municipal, lo cual es aceptable como simplificación en el contexto académico de este análisis.
    \item \textbf{Hosting:} El costo de \$13.500/mes corresponde a una estimación inicial para una infraestructura básica. Para un entorno de producción con redundancia y base de datos gestionada, un valor realista sería de \$50.000--\$100.000~CLP/mes.
\end{itemize}

Bajo un escenario conservador que ajuste estos supuestos (ingresos con promedio de suscriptores, costos operativos escalados y churn del 2\%), el VAN se mantiene positivo, confirmando la viabilidad económica del proyecto incluso bajo condiciones menos favorables.

\section{Conclusión de Factibilidad}
Gracias al análisis realizado, se concluye que el proyecto es económicamente viable. El VAN obtenido asciende a \textbf{\$158.219.839} utilizando una tasa de descuento del 10\%, y la TIR estimada es de aproximadamente \textbf{125\%}, significativamente superior a la tasa de descuento. Estos resultados indican que el proyecto no solo recupera su inversión inicial, sino que genera un excedente significativo. Asimismo, el flujo de caja proyectado alcanza resultados positivos a partir del segundo año, reforzando la sostenibilidad financiera a largo plazo. Si bien el análisis incorpora supuestos simplificadores (sección~4.3.2), incluso bajo un escenario conservador el proyecto mantiene viabilidad económica.